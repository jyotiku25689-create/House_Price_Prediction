...
with left:
    ...
    inr_rate = 83
    predicted_price_inr = predicted_price * inr_rate
    st.divider()
...
chart_data = df[["sqft_living", "price"]].dropna().sample(
    n=min(1000, len(df)),
    random_state=42
)
...
fig.update_layout(
    ...
    xaxis_title="House Area (sq ft)",
    yaxis_title="House Price (USD)"
)

st.plotly_chart(fig, width="stretch", key="area_price_chart")
st.subheader("🛏️ Bedrooms vs House Price")

bedroom_data = df[["bedrooms", "price"]].dropna()
...
fig2.add_trace(
    go.Bar(
        x=bedroom_analysis["bedrooms"],
        y=bedroom_analysis["price"],
        name="Average Price"
    )
)
...
fig2.update_layout(
    ...
    yaxis_title="Average House Price (USD)"
)

st.plotly_chart(fig2, width="stretch", key="bedroom_price_chart")
st.divider()

st.subheader("🤖 About the Machine Learning Model")

st.write("""
This project uses a Random Forest Regression algorithm to predict house prices.
The model is trained using the King County House Sales dataset.
...
The model performance is evaluated using R² Score and Mean Absolute Error (MAE).
""")